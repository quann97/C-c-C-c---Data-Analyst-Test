...
-- Step 1: calculate changes in headcount overtime by creating new column filled with '1' value for each hire date and '-1' value for each termination date, null values mean no changes, then put together all the dates and added values.
-- Step 2: calculate a accumulative sum of the changes to get the total number of employees at any point in time.
